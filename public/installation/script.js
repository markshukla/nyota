...
$(".submit").click(function(){
	...
	$.ajax({
      type: "POST",
      url: "/databasesetup",
      data: { username : username, code : code, base : base ,db_host : db_host ,db_name : db_name ,db_username : db_username ,db_password : db_password },
      headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
      success: function(data) {
          try {
              var response = JSON.parse(data);
              if(response.code == 404){
                  document.getElementById("errorTv2").innerHTML = response.description;
              }else if(response.code == 200){
                  window.location = "/login";
              }
          } catch (e) {
              document.getElementById("errorTv2").innerHTML = data;
          }
      },
      error (data) {
