                ...
                <div class="d-flex align-items-center">
                   <h6 class="text-white" >Political Posts</h6>
                    ...
                    <div class="ms-auto mt-3">
                        <a href="{{ route('political.create')}}" class="btn btn-success "><i class="fas fa-plus"></i> Add Post</a>
                    </div>
                </div>
            </div>
                ...
                <div class="col-md-4">
                  <div class="form-group p-2">
                    <select class="form-control selectpicker" onchange="searchByCategoryID()" id="category">
                        @if(empty($category)) 
                        <option selected="selected" disabled="disabled">Select Party</option> 
                        @else
                ...
                <div class="ms-auto m-3 d-flex">
                    ...
                    <div class="dropdown">
                      ...
                      <form action="{{ url('/posts-action') }}" method="POST" id="form-select-one">
                          ...
                          <input type="hidden" name="from_name" value="political">
                      </form>
                    </div>
                </div>
                        ...
                        <div style="background-image: linear-gradient(transparent, transparent, transparent, transparent, transparent, #302d2d, black);   border-radius: 17px;" class="card-body position-relative z-index-1 p-3">
                            ...
                            <div @if($post->json != '') class="mt-8" @else class="mt-9" @endif>
                                ...
                                <div class="d-flex mt-2" >
                                  <div>
                                    <a class="btn btn-icon-only btn-rounded btn-success mb-0 me-2 btn-sm d-flex align-items-center justify-content-center" href="{{secure_url('/political/'.$post->id.'/edit')}}">
                                ...
                                <form action="{{ url('political/'.$post->id) }}" method="POST" id="form-{{ $post->id }}">
                                    @method('DELETE')
                                    @csrf
...
<script>
    ...
    function searchByCategoryID(){
        ...
        if(id == "Show All"){
            window.location.replace("{{ url('/political') }}");
        }else if(id != "Select Party"){
            window.location.replace("{{ url('/politicalCategory') }}" + "/" + id);
        }
    }
