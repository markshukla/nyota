                ...
                <div class="d-flex align-items-center">
                   <h6 class="text-white" >Frames</h6>
                    ...
                    <div class="ms-auto mt-3">
                        <a href="{{ route('frame.create')}}" class="btn btn-success "><i class="fas fa-plus"></i> Add Frame</a>
                    </div>
                </div>
            </div>
            ...
            <div class="card-body mt-n3">
                ...
                <div class="row mb" >
                    ...
                    <div class="col-xl-3 col-l-3 col-sm-4 p-2  bg-transparent" >
                      ...
                      <div style="background:gray;background-image: url(@if($frame->thumbnail) {{url($frame->thumbnail)}} @else {{url('/images/placeholder.jpg')}} @endif);height:240px;background-size: cover;" class="border-radius-xl">
                        ...
                        <div class="card-body position-relative z-index-1 p-3">
                           <div style="width: 100%;height: 56px;position: absolute;left: 0;border-radius: 16px 16px 0 0px;top: 0;background-color: rgb(12 12 12 / 17%);z-index: -1;"></div>
                            <input type="checkbox" name="post_ids[]" value="{{$frame->id}}" class="post_ids p-2" style="float: right;width: 18px;height: 18px;">
                            <label style="background:green;border-radius:20px;" class="text-white text-sm p-1 m-0" >{{$frame->ratio}}</label>
                            ...
                            <div class="" >
                                ...
                                <div class="d-flex" >
                                  <div>
                                    <a class="btn btn-icon-only btn-rounded btn-success mb-0 me-2 btn-sm d-flex align-items-center justify-content-center" href="{{secure_url('/frame/'.$frame->id.'/edit')}}">
                                        <i class="fas fa-pencil-alt"></i>
                                    </a>
                                  </div>
                                  
                                  <div>
                                    <button class="btn btn-icon-only btn-rounded btn-danger mb-0 me-2 btn-sm d-flex align-items-center justify-content-center"  data-id="{{$frame->id}}" data-toggle="modal" data-target="#singleDeleteModal">
                                  ...
                                  <div class="form-switch align-items-center justify-content-center" >
                                     <input class="form-check-input status-switch" type="checkbox" data-id="{{$frame->id}}" @if($frame->status==0) checked @endif>
                                  </div>
                                  
                                  <div onclick="cheakPremium({{ $frame->id }})" class="align-items-center justify-content-center">
                                     <button class="btn btn-sm btn-premium btn_cust" style="width:60px;padding:2px;margin:2px" id="pre-{{ $frame->id }}">
                                         @if($frame->premium==0) Free @else Premium @endif
                                         </button>
                                  </div>
                                </div>
                            
                                <form action="{{ url('frame/'.$frame->id) }}" method="POST" id="form-{{ $frame->id }}">
                                    @method('DELETE')
                                    @csrf
