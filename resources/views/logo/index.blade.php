          ...
          <div class="card mb-4">
            ...
            <div class="card-header py-0 bg-primary">
                <div class="d-flex align-items-center">
                   <h6 class="text-white" >Logos</h6>
                    
                    <div class="ms-auto mt-3">
                        <a href="{{ route('logos.create')}}" class="btn btn-success "><i class="fas fa-plus"></i> Add New</a>
                    </div>
                </div>
            </div>
            ...
            <div class="card-body mt-n3">
                ...
                <div class="row mb" >
                    ...
                    <div class="col-xl-2 col-l-3 col-sm-4 p-2  bg-transparent" >
                      ...
                      <div style="background:gray;background-image: url(@if($sticker->thumb_url) {{url($sticker->thumb_url)}} @else {{url('/images/placeholder.jpg')}} @endif);height:140px;background-size: cover;" class="border-radius-xl">
                        ...
                        <div class="card-body position-relative z-index-1 p-2">
                            ...
                            <h5 class="text-white text-sm mt-0 pb-0" >{{$sticker->title}}</h5>
                            ...
                            <div class="mt-5" >
                                ...
                                <div class="d-flex mt-1" >
                                  <div>
                                    <a class="btn btn-icon-only btn-rounded btn-success mb-0 me-2 btn-sm d-flex align-items-center justify-content-center" href="{{secure_url('/logos/'.$sticker->id.'/edit')}}">
                                ...
                                <form action="{{ url('logos/'.$sticker->id) }}" method="POST" id="form-{{ $sticker->id }}">
                                    @method('DELETE')
                                    @csrf
                ...
                <div class="d-flex justify-content-center">{{ $logos->links() }}</div>
...
<script>
    ...
    function searchByCategoryID(){
        ...
        if(id == "Show All"){
            window.location.replace("{{ url('logos') }}");
        }else if(id != "Select Category"){
            window.location.replace("{{ url('logosCategory') }}" + "/" + id);
        }
    }
    ...
    function cheakPremium(id){
        ...
        $.ajax({
          type: "POST",
          url: "{{ secure_url('/logos-premium-action') }}",
          data: { id : id, type : cheak},
          headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
          success: function(data) {
              if(cheak == 0){
                  toastr.success("Logo set free");
              }else{
                  toastr.success("Logo set premium");
              }
          },
          error (data) {
    ...
    $(".status-switch").change(function(){
        ...
        $.ajax({
          type: "POST",
          url: "{{ secure_url('/logos-status') }}",
          data: { checked : checked , id : id},
          headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
          success: function(data) {
              
              if(checked){
                  toastr.success("Logo Activated");
              }else{
                  toastr.success("Logo Deactivated");
              }
          },
          error (data) {
              toastr.error(JSON.stringify(data));
