                ...
                <div class="d-flex align-items-center">
                   <h6 class="text-white" >Sections</h6>
                    ...
                    <div class="ms-auto mt-3">
                        <a href="{{ route('section.create')}}" class="btn btn-success "><i class="fas fa-plus"></i> Add New</a>
                    </div>
                </div>
            </div>
                  ...
                  <tbody>
                    @foreach ($sections as $section)
                       <tr id="{{ $section->id }}">
                        ...
                        <form action="{{ url('section/'.$section->id) }}" method="POST" id="form-{{ $section->id }}">
                            @method('DELETE')
                            @csrf
...
<script>
    ...
    $(".status-switch").change(function(){
        ...
        $.ajax({
          type: "POST",
          url: "{{ secure_url('/section-status') }}",
          data: { checked : checked , id : id},
          headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
          success: function(data) {
              
              if(checked){
                  toastr.success("Section Activated");
              }else{
                  toastr.success("Section Deactivated");
              }
          },
          error (data) {
              toastr.error(JSON.stringify(data));
